(() => {
  ...
  class SoundSystem {
    ...
    createReverb() {
      ...
      this.reverbNode = {
        delay: this.audioContext.createDelay(delayTime),
        feedback: this.audioContext.createGain(),
        wet: this.audioContext.createGain()
      };

      this.reverbNode.delay.delayTime.value = delayTime;
      this.reverbNode.feedback.gain.value = feedback;
      this.reverbNode.wet.gain.value = 0.15;

      this.reverbNode.delay.connect(this.reverbNode.feedback);
      this.reverbNode.feedback.connect(this.reverbNode.delay);
      this.reverbNode.delay.connect(this.reverbNode.wet);
      this.reverbNode.wet.connect(this.audioContext.destination);
    }

    // 音调随机化
    randomizePitch(baseFreq, variance = 0.1) {
      const randomFactor = 1 + (Math.random() - 0.5) * 2 * variance;
      return baseFreq * randomFactor;
    }

    // 创建带滤波器的振荡器
    createOscillatorWithFilter(type, startFreq, endFreq, duration, filterStartFreq = 3000, filterEndFreq = 500, addReverb = false) {
      const oscillator = this.audioContext.createOscillator();
      const gainNode = this.audioContext.createGain();
      const filter = this.audioContext.createBiquadFilter();

      oscillator.type = type;
      oscillator.frequency.setValueAtTime(this.randomizePitch(startFreq), this.audioContext.currentTime);
      oscillator.frequency.exponentialRampToValueAtTime(this.randomizePitch(endFreq), this.audioContext.currentTime + duration);

      filter.type = 'lowpass';
      filter.frequency.setValueAtTime(filterStartFreq, this.audioContext.currentTime);
      filter.frequency.exponentialRampToValueAtTime(filterEndFreq, this.audioContext.currentTime + duration);
      filter.Q.value = 1;

      oscillator.connect(filter);
      filter.connect(gainNode);

      if (addReverb && this.reverbNode) {
        gainNode.connect(this.audioContext.destination);
        gainNode.connect(this.reverbNode.delay);
      } else {
        gainNode.connect(this.audioContext.destination);
      }

      // 使用 setTargetAtTime 实现指数衰减
      gainNode.gain.setValueAtTime(0.3, this.audioContext.currentTime);
      gainNode.gain.setTargetAtTime(0.01, this.audioContext.currentTime, duration / 3);
    ...
    // 创建带低频增强的白噪声
    createNoiseWithBassBoost(duration, addReverb = false) {
      ...
      gainNode.gain.setTargetAtTime(0.01, this.audioContext.currentTime, duration / 4);
    ...
    createShootSound() {
      this.createOscillatorWithFilter('sawtooth', 800, 400, 0.1, 2500, 600);
    }

    createHitSound() {
      this.createOscillatorWithFilter('square', 200, 50, 0.05, 2000, 400);
    }

    createExplosionSound() {
      this.createNoiseWithBassBoost(0.3, true);
    }

    createFrostSound() {
      this.createOscillatorWithFilter('sine', 1200, 600, 0.2, 3500, 800);
    }

    createLightningSound() {
      this.createNoiseWithBassBoost(0.1, false);
    }

    createHealSound() {
      this.createOscillatorWithFilter('sine', 400, 800, 0.3, 2000, 1000);
    }

    createPortalSound() {
      this.createOscillatorWithFilter('sawtooth', 100, 50, 0.5, 1500, 300);
    }

    createWallSound() {
      this.createOscillatorWithFilter('triangle', 300, 600, 0.1, 2200, 700);
    }
    ...
    createLevelCompleteSound() {
      ...
      notes.forEach((freq, index) => {
        setTimeout(() => {
          this.createOscillatorWithFilter('sine', freq, freq, 0.3, 3000, 1200, true);
        }, index * 100);
      });
    }
    ...
    createGameOverSound() {
      ...
      notes.forEach((freq, index) => {
        setTimeout(() => {
          this.createOscillatorWithFilter('sawtooth', freq, freq, 0.4, 2000, 500, true);
        }, index * 150);
      });
    }
